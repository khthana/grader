...
function req(courseId: number, token?: string): NextRequest {
  const r = new NextRequest(`http://localhost/api/courses/${courseId}/gradebook`, { method: "GET" })
...
describe("GET /api/courses/[id]/gradebook", () => {
  ...
  it("returns 200 with correct gradebook matrix", async () => {
    ...
    const { gradebook } = await res.json()
    expect(gradebook.problems).toHaveLength(1)
    expect(gradebook.problems[0].title).toBe("Q1")
    expect(gradebook.problems[0].pointsMax).toBe(10)

    expect(gradebook.students).toHaveLength(1)
    expect(gradebook.students[0].idCode).toBe("64010001")
    expect(gradebook.students[0].scores[problemId]).toBe(8)
  })
})
