...
- **Unit Test Mode** *(#53):* `problem_type = 'unit'` — students write a named function; grade route dispatches to `runUnitTestCases(functionName, testCases, studentCode)` in `piston.ts`; Python harness calls the function directly, stdout protocol `PASS`/`FAIL:<repr>`/`ERROR:<msg>`; `test_cases.input` holds args literal, `test_cases.expected_output` holds expected return literal. Schema: `problems.problem_type TEXT DEFAULT 'io'` + `problems.function_name TEXT` + `problems.starter_code TEXT`. **UI:** ProblemEditor sidebar has I/O / Unit Test toggle; unit mode reveals Function Name field, hides I/O spec fields, relabels test case columns to "Arguments"/"Expected Return Value", hides "รันเฉลย"; Starter Code CodeMirror card (both modes); Code Policy card with blacklist/whitelist tag inputs; student page pre-populates `CodeEditor` with `starterCode`.
- **AI Generation for Unit Test Mode** *(#54):* `generateTestPlan` gains `problemType?: 'io' | 'unit'` param; unit mode prompt returns `{ solution, tests: [{args, expected_return}] }`; io mode unchanged. `generate` route passes `problemType` from problem record or request body. Exports `IoTestPlan` | `UnitTestPlan` union type.

Specs: `requirement/PRD.md` + `requirement/PRD-week-release.md` + `requirement/PRD-reference-solution-ai.md` + `requirement/PRD-profile.md` + `requirement/PRD-unit-test-blacklist.md`. Design rationale: `CONTEXT.md` (glossary), `docs/adr/`. DB migration scripts: `scripts/migrate-001-natural-keys.sql`, `scripts/migrate-002-week-is-released.sql`, `scripts/migrate-003-problem-reference-solution.sql`, `scripts/migrate-004-user-nickname.sql`, `scripts/migrate-005-unit-test-blacklist.sql`.
...
- **AI generation:** `src/lib/llm/index.ts` exports `generateTestPlan(problem) → IoTestPlan | UnitTestPlan` and `LlmNotConfiguredError`. `problemType?: 'io' | 'unit'` in the problem param selects prompt + return shape: io → `{ solution, inputs[] }`; unit → `{ solution, tests: [{args, expectedReturn}] }`. Uses `ANTHROPIC_API_KEY || LLM_API_KEY`; model via `LLM_MODEL` (default `claude-haiku-4-5-20251001`). `POST .../problems/generate` (`manage:true`) accepts `{ problemId }` (edit mode) OR `{ title, description, inputSpec?, outputSpec?, problemType? }` (create mode). Returns 503 when no key. "สร้างด้วย AI" button in ProblemEditor hides after 503; on success replaces all test cases + fills refSolution.
- **Grading** (`POST /api/grade`): `mode:run` = visible tests only, no Submission; `mode:submit` = all tests, stores Submission, enforces `close_at` (403 if past), checks enrollment, sets `is_late` if past `due_at`. Grading order: (1) code policy check — violation returns 200 `pointsEarned=0`, no Piston call; (2) dispatch: `problemType='unit'` → `runUnitTestCases`; else → `runTestCases`; (3) score = sum of `test_cases.score` for passing cases. Returns `GradeResult = { pointsEarned, pointsMax, totalTests, passedTests, results[], feedback }`.
...
- **Unit test harness** (`piston.ts#runUnitTestCases`)**:** builds per-case Python harness that calls `functionName(argsLiteral)`, compares result to `expectedReturn`, prints `PASS`/`FAIL:<repr>`/`ERROR:<msg>` to stdout. `tc.input` = Python args literal; `tc.expectedOutput` = expected return literal.
