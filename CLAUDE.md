...
- **Unit Test Mode** *(#53):* `problem_type = 'unit'` — students write a named function; grade route dispatches to `runUnitTestCases(functionName, testCases, studentCode)` in `piston.ts`; Python harness calls the function directly, stdout protocol `PASS`/`FAIL:<repr>`/`ERROR:<msg>`; `test_cases.input` holds args literal, `test_cases.expected_output` holds expected return literal. Schema: `problems.problem_type TEXT DEFAULT 'io'` + `problems.function_name TEXT` + `problems.starter_code TEXT`. **UI (ProblemEditor) not yet implemented.**
