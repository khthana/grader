...
Features in progress (issues #50–#54 — not yet shipped):
- **Per-Test-Case Scoring** *(#51):* fix `POST /api/grade` to sum `test_cases.score` per passing case instead of all-or-nothing. `test_cases.score` column exists but is currently unused by the grading route.
- **Code Policy — Blacklist / Whitelist** *(#52, requires #50):* Instructor defines terms that must be absent (blacklist) or present (whitelist) per Problem. Pure `checkCodePolicy(code, blacklist, whitelist)` module uses whole-word regex; grade route rejects before Piston if violated; applies to both `mode:run` and `mode:submit`. Schema: `problems.blacklist TEXT[]` + `problems.whitelist TEXT[]`.
- **Unit Test Mode** *(#53, requires #50):* `problem_type = 'unit'` — students write a named function; grader calls it directly via a Python harness (1 Piston call/case); `test_cases.input` holds args literal, `test_cases.expected_output` holds expected return literal. Schema: `problems.problem_type TEXT DEFAULT 'io'` + `problems.function_name TEXT` + `problems.starter_code TEXT`. ProblemEditor gains type toggle, function name field, starter code editor, and relabelled test-case form.
- **AI Generation for Unit Test Mode** *(#54, requires #53):* `generateTestPlan` gains `problemType` param; unit mode prompt returns `{ solution, tests: [{args, expected_return}] }` instead of `{ solution, inputs[] }`.

Specs: `requirement/PRD.md` + `requirement/PRD-week-release.md` + `requirement/PRD-reference-solution-ai.md` + `requirement/PRD-profile.md` + `requirement/PRD-unit-test-blacklist.md`. Design rationale: `CONTEXT.md` (glossary), `docs/adr/`. DB migration scripts: `scripts/migrate-001-natural-keys.sql`, `scripts/migrate-002-week-is-released.sql`, `scripts/migrate-003-problem-reference-solution.sql`, `scripts/migrate-004-user-nickname.sql`, `scripts/migrate-005-unit-test-blacklist.sql` *(pending — created when #50 ships)*.
...
- **AI generation:** `src/lib/llm/index.ts` exports `generateTestPlan(problem) → { solution, inputs[] }` and `LlmNotConfiguredError`. Uses `ANTHROPIC_API_KEY || LLM_API_KEY`; model via `LLM_MODEL` (default `claude-haiku-4-5-20251001`). `POST .../problems/generate` (`manage:true`) accepts `{ problemId }` (edit mode, reads from DB) OR `{ title, description, inputSpec?, outputSpec? }` (create mode, raw fields). Returns 503 when no key. "สร้างด้วย AI" button in ProblemEditor hides after 503; on success replaces all test cases + fills refSolution. *(Planned #54: `generateTestPlan` gains `problemType` param; unit mode returns `{ solution, tests: [{args, expected_return}] }` instead of `{ solution, inputs[] }`.)*
- **Grading** (`POST /api/grade`): `mode:run` = visible tests only, no Submission; `mode:submit` = all tests, stores Submission, enforces `close_at` (403 if past), checks enrollment, sets `is_late` if past `due_at`. Calls Piston (`src/lib/piston.ts`, Python 3.10.0). Returns `GradeResult = { pointsEarned, pointsMax, totalTests, passedTests, results[], feedback }`. *(Planned #51: scoring becomes per-test-case sum of `test_cases.score`; currently all-or-nothing.)*
- **Two-tier deadline (ADR 0002):** `close_at` checked first → 403 if past; `due_at` → sets `is_late` flag.
- **Code policy** *(planned #52):* `checkCodePolicy(code, blacklist, whitelist)` pure module — whole-word regex; grade route calls it before Piston; violation → reject with flagged terms; applies to both run and submit modes.
- **Unit test harness** *(planned #53):* when `problem.problemType === 'unit'`, grade route dispatches to `runUnitTestCase(functionName, argsLiteral, expectedReturn, studentCode)` in `piston.ts`; harness wraps student code, calls the function, prints `PASS`/`FAIL:<repr>`/`ERROR:<msg>` to stdout.
